                ...
                <form action="{{ route('clients.update', $client) }}" method="post">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input required type="text" class="form-control" id="exampleInputName"
                               value="{{ $client->first_name }}" name="nombres">
                    </div>

                    <div class="form-group mb-3">
                        <label for="apellidos">Apellidos (opcional):</label>
                        <input type="text" class="form-control" id="apellidos" value="{{ $client->last_name }}"
                               name="apellidos">
                    </div>
                        ...
                        <div class="row">
                            <div class="col">
                                <select class="form-select" aria-label="Default select example"
                                    ...
                                    @if(!is_null($client->documentType?->id))
{{--                                        ENTER HERE IF MODEL HAS DOCUMENT--}}
                                        @foreach($documentsTypes as $documentType)
                                            @if($documentType->id == $client->documentType->id)
                                                <option selected
...
{{--                                        ENTER HERE IF MODEL DOES NOT HAVE DOCUMENT--}}
                            ...
                            <div class="col">
                                <input type="number" min="1" class="form-control" placeholder="Ejemplo: 999999999"
                                       name="documento" id="documento" value="{{ $client->document }}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="exampleFormControlTextarea1" rows="3"
                                  name="comentario">{{ $client->comment }}</textarea>
                    </div>
